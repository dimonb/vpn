...
from .utils import netset_expand, dedupe_lines
...
class TemplateProcessor:
    ...
    async def smart_fetch(self, url_str: str, incoming_host: str, request_headers: Dict) -> str:
    ...
    async def expand_netset(self, url_str: str, suffix: str) -> List[str]:
        ...
        try:
            ...
            print(f"NETSET expanded {len(expanded)} entries (IPv4→/{settings.ipv4_block_prefix}, IPv6→/{settings.ipv6_block_prefix})")
            return expanded
    ...
    def parse_template(self, template_text: str) -> Tuple[List[Dict], List[Optional[str]], int]:
    ...
    async def expand_rule_set(self, task: Dict, incoming_host: str, request_headers: Dict) -> List[str]:
        ...
        try:
            ...
            if netset_urls:
                print(f"Found {len(netset_urls)} NETSET entr{'ies' if len(netset_urls) > 1 else 'y'} in {url}")
            ...
            for line in lines:
                ...
                if re.search(r'(,PROXY|,DIRECT|,REJECT)\s*$', trimmed, re.IGNORECASE):
                    trimmed = re.sub(r',(PROXY|DIRECT|REJECT)\s*$', suffix, trimmed, flags=re.IGNORECASE)
                else:
    ...
    async def process_template(self, tpl_text: str, incoming_host: str, request_headers: Dict) -> str:
        ...
        expansions = await asyncio.gather(
            *[self.expand_rule_set(task, incoming_host, request_headers) for task in tasks]
        )
