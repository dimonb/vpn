...
"""
    ...
    @pytest.fixture
    def clash_processor(
        self, mock_template_processor: TemplateProcessor, proxy_config: ProxyConfig
    ) -> ClashProcessor:
        """Create ClashProcessor instance with ProxyConfig."""
        return ClashProcessor(mock_template_processor, proxy_config)

    def test_replace_proxy_placeholders_default(
        self, clash_processor: ClashProcessor, sample_clash_yaml: str
    ) -> None:
        ...
        request_headers = {}
        updated_config = clash_processor.replace_proxy_placeholders(
            clash_config, request_headers
        )
    ...
    def test_replace_proxy_placeholders_premium(
        ...
        assert isinstance(updated_config["proxies"], list)
        assert (
            len(updated_config["proxies"]) == 1
        )  # 1 proxy (one per proxy, not per user)
    ...
    def test_replace_proxy_placeholders_no_proxy_config(
        self, mock_template_processor: TemplateProcessor, sample_clash_yaml: str
    ) -> None:
    ...
    def test_replace_proxy_placeholders_no_placeholders(
...
"""
    ...
    @pytest.mark.asyncio
    async def test_process_clash_config_with_proxy_placeholders(
        self, clash_processor: ClashProcessor, sample_clash_yaml: str
    ) -> None:
    ...
    def test_proxy_config_structure(
        self, mock_settings, clash_processor: ClashProcessor, sample_clash_yaml: str
    ) -> None:
        ...
        hysteria2_configs = [
            p for p in updated_config["proxies"] if p["type"] == "hysteria2"
        ]
